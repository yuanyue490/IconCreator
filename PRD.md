...
- **匹配模式（开源组匹配）**：输入一组词 → AI 智能匹配出一组**同库同款**的开源图标
...
| 维度 | 传统搜索（Iconify 等） | IconCraft 组匹配 |
|---|---|---|
| 输入 | 单个关键词 | 多词 / 句子 / 列表 |
...
- 用户可随时切换 `{库, 风格}`，组合结果即时重新匹配并渲染
- 支持"AI 帮我选"：根据输入的语义倾向自动推荐（如"极简科技感" → Lucide；"活泼"→ Phosphor Duotone）
...
- 列表式展示：每一行 = `词 → 图标 → 图标名 → 单张操作`
- 未匹配项灰色标注，提示"该库未收录 · 可尝试切换图标库"
- 顶部显示"来源：Lucide · 线性 · 成功 6/6"

**批量操作**：

- **一键下载整组**：
  - SVG ZIP 包
  - PNG ZIP 包（可选尺寸：24/32/48/64）
  - 单 SVG Sprite 文件（含 `<symbol>` 定义，一次引入全部）
  - React 组件集合（每个图标一个 `.tsx`，附 `index.ts` barrel export）
  - Vue 组件集合（同上）
- **复制整组 SVG 代码**
- **重新匹配**：保留输入，换一个库/风格
...
- **状态**：Zustand
- **图标渲染**：`@iconify/react`
- **图像处理**：Canvas API（SVG → PNG）
- **打包下载**：JSZip

### 3.2 后端技术栈

- **服务端**：Node.js + Fastify（轻量，低延迟）
- **缓存**：Redis（图标元数据 + 匹配结果缓存）
...
```plaintext
┌──────────────────────────────────────────────────────────┐
│  [Logo] IconCraft              [浏览全部] [历史] [登录]  │ ← 48px 细边导航
├──────────────────────────────────────────────────────────┤
│                                                          │
│  ┌────────────────────────────────────────────────────┐ │
│  │ [模式: ◉ AI 生成  ○ 组匹配]                        │ │
...
- **模式切换**：顶部两个 radio-style 胶囊，切换时输入框 placeholder 和右侧"风格选择器"同步变化
- **输入框**：
  - 自适应高度（单行 → 多行）
  - `Enter` 提交，`Shift+Enter` 换行
  - 多词自动高亮（匹配模式下解析后每个词一个 chip）
- **参数区**：
  - AI 模式：5 种风格 chip + 高级选项（折叠）
  - 匹配模式：`{图标库, 风格}` 二级下拉 + "AI 帮我选"按钮
...
- 4 宫格结果，hover 浮层操作
- 提示词展示行（小字，可展开 / 复制）
- 底部操作条：下载全部 / 重新生成 / 收藏全部

### 4.5 匹配模式输出区

```plaintext
来源：Lucide · 线性   匹配：6/6   耗时：1.8s    [重新匹配] [下载全部▼]
...
[下载全部] 弹出菜单：SVG ZIP / PNG ZIP / SVG Sprite / React / Vue / 复制代码
```

- 行悬停高亮，点击行打开详情侧边栏（大图预览 + 替换候选）
- 未匹配行突出显示，提供"换个库"和"手动指定"两种 recover 操作
- 支持拖拽行调整顺序（影响导出的 index 顺序）
...
- **首屏加载**：< 1.5 秒（无 hero，资源更少）
- **AI 3D 生成响应**：20–40 秒（标准）
- **AI 组匹配响应**：< 2.5 秒（LLM + 图标加载）
- **SVG → PNG 转换**：< 1 秒
- **批量打包下载**：< 3 秒（50 个图标）
...
- 接入 Lucide + Heroicons（2 库 3 风格起步，覆盖 80% 场景）
- LLM Prompt 直调方案（方案 A）
- 批量下载：SVG ZIP / PNG ZIP / SVG Sprite
- 降级：本地同义词词典兜底
...
- 增加 Phosphor、Tabler、Feather、Material Symbols
...
| 产品 | 类型 | 优势 | 劣势 | IconCraft 差异化 |
|---|---|---|---|---|
| **Midjourney** | AI 生图 | 图像质量顶级 | 通用工具，非图标专用，无组匹配 | 垂直聚焦图标，内置专业 prompt，输出可直接用 |
| **Iconify** | 开源聚合 | 图标数量最多 | 仅单个搜索展示，无批量，无 AI 语义 | **AI 组匹配 + 风格一致性硬约束** |
| **Icons8** | 素材库 | 风格统一，质量高 | 收费，无 AI 生成，无批量匹配 | AI 生成 + 免费开源，组匹配 |
...
**文档版本**：v3.0（定位重构）
**创建日期**：2026-04-17
**最新修订**：2026-04-17（从"双核心图标工具平台"重构为"AI 图标工作台，双模式单输入"）
**负责人**：产品团队
**审核状态**：待评审
