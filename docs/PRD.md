...
| 维度 | 传统搜索（Iconify 等） | IconCraft SVG匹配 |
|---|---|---|
| 输入 | 单个关键词 | 多词 / 句子 / 列表 |
...
**当前开发版实现说明（2026-04-20）**：

- 前端已开放 `baseURL`、`apiKey`、`model`、`systemPrompt` 配置，便于切换兼容 OpenAI 协议的模型服务并手动微调匹配策略。
- 后端当前匹配链路已落地为：`本地词典精确匹配 -> LLM 语义匹配 -> 本地兜底匹配 -> 未匹配`。
- LLM 返回结果会做本地图标名校验，只有命中当前 `{库, 风格}` catalog 的图标才会进入最终结果。
...
- 结果区已采用卡片平铺网格，而非行列表格。
- 每张卡片展示：图标预览、原始词、实际图标名、命中来源标签。
- 已实现单张 `下载 SVG`、`复制 SVG`、`详情预览`。
- 已实现整组导出当前匹配结果；更完整的 ZIP / Sprite / 组件代码导出仍在后续阶段。
- 结果概览中会显示 `本地词典 / LLM 语义 / 本地兜底 / 未匹配` 的来源统计。
