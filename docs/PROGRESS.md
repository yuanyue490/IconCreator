...
> **日期**：2026-05-13
> **阶段**：v1.0 发布版 · 数字孪生体验重排 · AI 自定义提示词风格
...
- **数字孪生布局重排**：数字孪生模式收敛为上下两段工作流：第一段聚焦需求对话与底部贯通输入，第二段「生成图片参考」承载提示词编辑、生成按钮与图片结果。提示词编辑从第一段右侧移入生图区域，使「编辑提示词 -> 生成图片」成为连续动作。
- **需求摘要低干扰化**：需求摘要从固定内容区改为对话框内右上角半透明浮层；摘要不参与对话内容排版，展开后不再导致对话框整体高度突变。摘要内容改为更紧凑的字号、行距和间距，降低视觉层级。
- **数字孪生文案减噪**：移除多处解释性提示，只保留顶部一句必要引导与关键错误反馈；提示词编辑区去掉「恢复」按钮，仅保留复制能力，减少非核心操作干扰。
- **AI 生成 · 自定义提示词风格**：`AiGenerateSection` 的「提示词风格」新增 **自定义** Chip。选中后可填写「主体提示词模板」与「规避内容」，支持 `{生成物体}`、`{主色调}`、`{材质}` 占位符替换；提示词预览、生成请求与历史记录均使用该自定义模板，并记录风格名为「自定义」。
- **验证**：本轮前端改动已通过 `tsc --noEmit -p frontend/tsconfig.json` 与 `vite build`。
